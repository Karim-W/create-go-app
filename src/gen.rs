...
fn generate_server_from_swagger(path:&str) {
    ...
    let swagger: models::OpenApi = serde_json::from_str(&content).unwrap();
    ...
    for (key, value) in swagger.paths {
        ...
        if let Some(get_operation) = value.get {
            ...
            let first_tag = req.tags.clone().unwrap_or(vec!["default".to_string()]).first().unwrap().clone();
            // check if tag exists
            if !tree.handlers.contains_key(first_tag.clone().as_str()) {
                tree.handlers.insert(first_tag.clone(), vec![]);
            }
            // add to tag
            tree.handlers.get_mut(&first_tag.clone()).unwrap().push(req.clone());
        }
        // ==> Post
