...
func InitOrDie() {
	...
	config, err = cafe.New(cafe.Schema{
		"APP_INSIGHTS_INSTRUMENTATION_KEY": cafe.String("INSTRUMENTATION_KEY").Require(),
...
// GetAppInsightsInstrumentationKey returns the instrumentation key for app insights
// It is used to initialize the app insights tracer adapter
func GetAppInsightsInstrumentationKey() (string, error) {
	return config.GetString("APP_INSIGHTS_INSTRUMENTATION_KEY")
}
